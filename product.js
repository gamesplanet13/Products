...
function youtubeEmbed(url){const s=String(url||"");let m=s.match(/youtu\.be\/([^?&/]+)/)||s.match(/youtube\.com\/shorts\/([^?&/]+)/)||s.match(/[?&]v=([^?&/]+)/);return m?`https://www.youtube-nocookie.com/embed/${m[1]}`:""}
...
if(!p){document.getElementById("productPage").innerHTML=`<div class="panel empty"><h1>Product not found</h1><p>The product link may be outdated.</p><a class="checkout-btn" href="index.html">Return to store</a></div>`}
  ...
  <section class="detail-sections"><article class="panel"><h2>Product description</h2><p>${escapeHtml(p.description)}</p><p>This Games Planet listing is prepared with clear compatibility information and a pre-dispatch quality check.</p><h3>Videos & buying links</h3><div class="external-links">${(p.videos||[]).map((u,i)=>`<a href="${u}" target="_blank" rel="noopener">▶ YouTube ${i+1}</a>`).join("")}${(p.shops||[]).map(([n,u])=>`<a href="${u}" target="_blank" rel="noopener">${escapeHtml(n)}</a>`).join("")}<a href="${p.whatsapp}" target="_blank" rel="noopener">WhatsApp Order</a></div><h3>Warranty, cancellation & refund policy</h3><p>${escapeHtml(p.policy)}</p></article>
  <article class="panel"><h2>Product information</h2><table class="spec-table"><tr><th>Product name</th><td>${escapeHtml(p.name)}</td></tr><tr><th>SKU</th><td>${escapeHtml(p.id.toUpperCase())}</td></tr><tr><th>Category</th><td>${escapeHtml(p.category)}</td></tr><tr><th>Condition</th><td>${escapeHtml(p.condition)}</td></tr><tr><th>Approx. size / pack size</th><td>${escapeHtml(p.dimensions)}</td></tr><tr><th>Approx. weight</th><td>${p.weight} g</td></tr><tr><th>Compatibility</th><td>${escapeHtml(p.compatibility)}</td></tr><tr><th>Price</th><td>${money(p.price)}</td></tr><tr><th>Policy</th><td><b>${escapeHtml(p.policy)}</b></td></tr></table></article>
  <article class="panel"><h2>Hardware details</h2><ul class="rich-list">${(p.hardware||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul><h2>Software details</h2><ul class="rich-list">${(p.software||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul></article>
  <article class="panel"><h2>Box contains</h2><ul class="rich-list">${(p.boxContents||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul><h2>Specifications</h2><ul class="rich-list">${(p.specifications||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul></article>
  <article class="panel technical-panel"><h2>Technical Details</h2><table class="spec-table">${(p.technicalDetails||[]).map((x,i)=>{const parts=String(x).split(":");return `<tr><th>${escapeHtml(parts.length>1?parts.shift():`Detail ${i+1}`)}</th><td>${escapeHtml(parts.length?parts.join(":").trim():x)}</td></tr>`}).join("")}</table></article></section>
  ${(p.videos||[]).length?`<section class="panel video-section"><h2>Product videos</h2><div class="video-grid">${p.videos.map((u,i)=>{const e=youtubeEmbed(u);return e?`<div><iframe src="${e}" title="${escapeHtml(p.name)} video ${i+1}" loading="lazy" allowfullscreen></iframe><a href="${u}" target="_blank" rel="noopener">Open on YouTube</a></div>`:""}).join("")}</div></section>`:""}
  ...
  document.body.insertAdjacentHTML("beforeend",`<div class="image-lightbox" id="imageLightbox" aria-hidden="true"><button type="button" id="imageClose" aria-label="Close image">×</button><img id="lightboxProductImage" alt="${escapeHtml(p.name)}"></div>`);
  document.getElementById("mainProductImage").onclick=()=>{const lb=document.getElementById("imageLightbox");document.getElementById("lightboxProductImage").src=document.getElementById("mainProductImage").src;lb.classList.add("open");lb.setAttribute("aria-hidden","false")};
  const closeLightbox=()=>{const lb=document.getElementById("imageLightbox");lb.classList.remove("open");lb.setAttribute("aria-hidden","true")};
  document.getElementById("imageClose").onclick=closeLightbox;
  document.getElementById("imageLightbox").onclick=e=>{if(e.target.id==="imageLightbox")closeLightbox()};
  renderReviews(p);
