...
function saveOrderSummary(c){
  const items=Object.entries(c).map(([id,quantity])=>{
    const p=GP_PRODUCTS.find(x=>x.id===id);if(!p)return null;
    return{name:p.name,quantity,unitPrice:p.price,lineTotal:p.price*quantity,unitWeight:p.weight||100,lineWeight:(p.weight||100)*quantity};
  }).filter(Boolean);
  localStorage.setItem("gp_order_summary",JSON.stringify({items}));
}
function addToCart(id,goCheckout=false){const c=readCart();c[id]=(c[id]||0)+1;writeCart(c);if(goCheckout){saveOrderSummary(c);location.href="order-prefill.html?source=cart"}else toast("Added to cart")}
